...
void init_font(const char* text, float font_size, float start_x, float start_y) {
    // Load font file
    ...
    // Start text at specified (start_x, start_y) in pixel coordinates (top-left origin)
    float x = start_x;
    float y = start_y + ascent; // Adjust for baseline (ascent moves text up to align top)
    ...
    for (size_t i = 0; i < strlen(text); i++) {
        ...
        // Normalize coordinates to Vulkan NDC ([-1, 1], top-left origin)
        float x0_norm = (x + x0) / (float)vkCtx.width * 2.0f - 1.0f;
        float x1_norm = (x + x1) / (float)vkCtx.width * 2.0f - 1.0f;
        float y0_norm = -1.0f + (y + y0) / (float)vkCtx.height * 2.0f; // Flip y to start at top
...
int main(int argc, char* argv[]) {
    ...
    create_triangle();
    create_pipeline();
    init_font("Hello Vulkan!", 32.0f, 10.0f, 10.0f); // Text at (10, 10) from top-left
