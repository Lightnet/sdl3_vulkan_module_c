...
// Add cimgui includes (assume you have downloaded and built cimgui with Vulkan and SDL3 backends enabled via the generator)
#include "cimgui.h"
...
void createCommandBuffers() {
    ...
    // Note: Recording moved to per-frame in main loop for dynamic ImGui rendering
}
...
int main(int argc, char* argv[]) {
    ...
    if (vkCreateDescriptorPool(device, &pool_info, NULL, &descriptorPool) != VK_SUCCESS) {
        printf("Failed to create descriptor pool for ImGui\n");
        exit(1);
    }
    ...
    init_info.Subpass = 0;
    init_info.MinImageCount = imageCount; // Use current imageCount as min for init
    init_info.ImageCount = imageCount;
    init_info.MSAASamples = VK_SAMPLE_COUNT_1_BIT;
    init_info.Allocator = NULL;
    init_info.CheckVkResultFn = NULL; // Optional error callback
    ...
    // Upload ImGui fonts
    {
        VkCommandBufferAllocateInfo alloc_info_temp = {VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO};
        alloc_info_temp.commandPool = commandPool;
        alloc_info_temp.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
        alloc_info_temp.commandBufferCount = 1;

        VkCommandBuffer command_buffer_temp;
        vkAllocateCommandBuffers(device, &alloc_info_temp, &command_buffer_temp);

        VkCommandBufferBeginInfo begin_info_temp = {VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};
        begin_info_temp.flags |= VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
        vkBeginCommandBuffer(command_buffer_temp, &begin_info_temp);

        vkEndCommandBuffer(command_buffer_temp);

        VkSubmitInfo submit_info_temp = {VK_STRUCTURE_TYPE_SUBMIT_INFO};
        submit_info_temp.commandBufferCount = 1;
        submit_info_temp.pCommandBuffers = &command_buffer_temp;

        vkQueueSubmit(graphicsQueue, 1, &submit_info_temp, VK_NULL_HANDLE);
        vkQueueWaitIdle(graphicsQueue);

        vkFreeCommandBuffers(device, commandPool, 1, &command_buffer_temp);
    }
    ...
    while (running) {
        ...
        // Example ImGui content (demo window)
        bool show_demo = true;
        ...
        // Record command buffer dynamically each frame
